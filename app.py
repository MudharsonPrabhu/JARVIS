...
def process():
    ...
    mode = request.json.get('mode', 'conversation')  # get mode from frontend
    ...
    model_name = (
        ...
        if mode == "coding"
        else "lmstudio-community/phi-3-mini-4k-instruct"
    )
    ...
    while thread.is_alive():
        if cancel_generation.is_set() or time.time() - start_time > timeout:
            break
        time.sleep(0.1)
...
def switch_model():
    ...
    # Path to .bat files for launching models
    bat_files = {
        "conversation": "C:/Users/mudha/OneDrive/Documents/JARVISLAUNCHLM/run_phi3.bat",
        "coding": "C:/Users/mudha/OneDrive/Documents/JARVISLAUNCHLM/run_codellama.bat"
    }

    bat_path = bat_files.get(mode)
    if not bat_path or not os.path.exists(bat_path):
        return jsonify({"error": "Invalid mode or batch file not found"}), 400

    try:
        # Kill any existing llama.cpp server
        os.system("taskkill /f /im server.exe >nul 2>&1")

        # Small delay to ensure it's fully killed
        time.sleep(1.5)

        # Launch the new model using the bat file
        subprocess.Popen(bat_path, shell=True)

        return jsonify({"status": f"{mode} model launch initiated."})
